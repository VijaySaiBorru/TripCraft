        ...
        with open(CSV_PATH, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if review_rows:
                    ...
                    # review summary → pick longest summary
                    summaries = [
    ...
    def build_prompt(
        ...
        for r in (restaurant_ref or []):
            ...
            restaurant_cards.append({
                ...
                # review signals
                "restaurant_quality": r.get("restaurant_quality"),
                "food_signal": r.get("food_signal"),
                "service_signal": r.get("service_signal"),
                "ambience_signal": r.get("ambience_signal"),
                "value_signal": r.get("value_signal"),
                "menu_variety_signal": r.get("menu_variety_signal"),

                "wait_risk": r.get("wait_risk"),
                "hygiene_risk": r.get("hygiene_risk"),
                "persona_alignment": r.get("persona_alignment"),
                "persona_utility": r.get("persona_utility"),
...
- persona-based signals (persona_alignment, persona_utility) — higher means better fit for this persona
- aggregate_rating and review-based quality signals
- budget-friendliness (lower avg_cost is preferred when quality and persona fit are similar)
- suitability for a traveler (not hyper-local places)

Review and persona signals available on each restaurant:
- restaurant_quality: overall quality score.
- food_signal, service_signal, ambience_signal, value_signal, menu_variety_signal: aspect scores (higher is better).
- wait_risk, hygiene_risk: risk scores (lower is better).
- persona_alignment: how well this restaurant matches the user persona (higher is better).
...
- higher persona_alignment and persona_utility
- higher aggregate_rating and restaurant_quality
- better food/service/ambience/value/menu_variety signals
- lower wait_risk and hygiene_risk
- more budget-friendly (lower avg_cost) when persona fit and quality are similar
...
✔ Ranking is ONLY by persona + local suitability+review_signal.
