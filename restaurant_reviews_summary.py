...
output_path = "/scratch/sg/Vijay/TripCraft/TripCraft_database/reviews/restaurant_review_summary.csv"
...
unique_restaurants = df["restaurant_index"].unique()

print("Unique Restaurants:", len(unique_restaurants))
...
hygiene_keywords = [
"dirty","unclean","smell","rotten","hair","undercooked",
"cold food","sick","food poisoning","raw","spoiled"
]
...
for rest_index in tqdm(unique_restaurants):

    df_rest = df[df["restaurant_index"] == rest_index]

    restaurant_name = df_rest.iloc[0]["Name"]
    city = df_rest.iloc[0]["City"]
    state = df_rest.iloc[0]["State"]
...
Focus on food quality, service, ambience,  price/value, waiting time and hygiene issues.
...
"""
    ...
    final_score = core_score + 0.4 * domain_bonus - 0.3 * risk_penalty
    ...
    results.append({
        ...
        "City": city,
        "State": state,
        "restaurant_index": rest_index,
        "num_reviews": num_reviews,

        "restaurant_quality": round(final_score,4),
