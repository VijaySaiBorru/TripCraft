...
def is_valid_poi_sequence(question, tested_data, plan_id=None):
    ...
    for i, unit in enumerate(tested_data[:total_days]):
        ...
        if (not poi_list or poi_list == '-'):
            ...
            return None, f"PoI list missing on day {day_num}."
...
def is_valid_meal_gaps(question, tested_data):
    ...
    for i, day_plan in enumerate(tested_data[:question['days']]):
        ...
        if not poi_list or poi_list == '-':
            return None, f"PoI list missing on day {day}."
...
def is_not_absent(question, tested_data):
    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        # Required keys (lenient check: if missing, assume '-')
        ...
        if ('from ' in unit.get('current_city', '') or 'to ' in unit.get('current_city', '')):
            if unit.get('transportation') in ['', '-', None]:
        ...
        if ('from ' not in unit.get('current_city', '') and ' to ' not in unit.get('current_city', '')):
            if unit.get('attraction') in ['', '-', None]:
        ...
        if day != question['days'] and unit.get('accommodation') in ['', '-', None]:
