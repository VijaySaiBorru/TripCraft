...
"""
    ...
    def validate(self, data: Dict[str, Any], days: int):
        ...
        legs = data["legs"]
        # print("Validating strategy:", strategy)
        # print("Validating legs:", legs)
        # print("Against transport_ref:", self._transport_ref)
    ...
    def _is_from_reference(self, details):
        ...
        for leg in self._transport_ref["legs"]:
            for mode, mode_obj in leg["modes"].items():
                ...
                # ✅ FIX: match by flight_number instead of full dict
                if mode == "flight":
                    for f in mode_obj:
                        if f["flight_number"] == details.get("flight_number"):
                            return True

                # taxi / self-driving (keep same)
                else:
                    if details == mode_obj:
                        return True
    ...
    def build_transport_ref(
        ...
        for idx, ((frm, to), date) in enumerate(zip(legs, travel_dates), start=1):
            ...
            if idx == 1:
                flights = sorted(
                    flights,
                    key=lambda x: (x["arrival_time"], x["price"])
                )[:10]

            elif idx == len(legs):
                flights = sorted(
                    flights,
                    key=lambda x: (-int(x["departure_time"].replace(":", "")), x["price"])
                )[:10]
    ...
    def choose_transport(
        ...
        # print("Initial validation:", "OK" if ok else f"FAIL - {msg}")
