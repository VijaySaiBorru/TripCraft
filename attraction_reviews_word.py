...
print("Unique attractions:", df["Name"].nunique())
print("\n==============================\n")
...
extra_stopwords = {
    "place","great","nice","really","just","time","good",
    "like","little","also","one","would","us","visit","visited"
}
...
print("TOP 40 ATTRACTION SIGNAL WORDS\n")

for w, c in word_counts.most_common(40):
    print(w, ":", c)

pd.DataFrame(word_counts.most_common(200), columns=["word","count"]).to_csv(
    "attraction_word_frequency.csv", index=False
)

print("\nSaved: attraction_word_frequency.csv")
...
print("TOP 40 ATTRACTION PHRASES\n")

for phrase, count in phrase_freq[:40]:
    print(phrase, ":", count)

pd.DataFrame(phrase_freq[:200], columns=["phrase","count"]).to_csv(
    "attraction_phrases.csv", index=False
)

print("\nSaved: attraction_phrases.csv")
...
print("DISCOVERED ATTRACTION REVIEW TOPICS\n")
...
pd.DataFrame(topics).to_csv("attraction_topics.csv", index=False)

print("\nSaved: attraction_topics.csv")