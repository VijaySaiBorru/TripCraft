...
"""
    ...
    """
        ...
        Define bounds:
        - earliest_allowed = max(day_anchor, 1920)
        - latest_allowed_end = min(2070, day_end)

        Define candidates:
        - Candidate A (IDEAL) = 2010
    ...
    """
        ...
        - earliest_allowed = max(
            2160,
            last_meal_end + MEAL_GAP if last_meal_end exists else 2160
        )
        - latest_end = min(2380, day_end)

        Define candidates:
        - Candidate A (IDEAL) = 2320
        - Candidate B (EARLIEST) = earliest_allowed
    ...
    """
        ...
        - earliest_allowed = max(
            2550,
            last_meal_end + MEAL_GAP if last_meal_end exists else 2550
        )
        - latest_allowed_end = min(2760, day_end)

        Define candidates:
        - Candidate A (IDEAL) = 2685
        - Candidate B (EARLIEST) = earliest_allowed
...
"""
        ...
        for idx, original_day in enumerate(days):
            ...
            # --------------------------------------------------
            if day_type == "FIRST_DAY":
                execution_prompt = self.build_first_day_execution_hints(day, day_type,attraction_duration_map)
                ...
                execution_prompt = self.build_non_travel_execution_hints(
                    ...
                    attraction_duration_map=attraction_duration_map,
